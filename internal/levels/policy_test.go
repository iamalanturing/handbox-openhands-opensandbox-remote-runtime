...
func TestPolicy_OfflineAndFullOmitField(t *testing.T) {
	for _, level := range []Level{Offline, Full} {
		policy, err := Policy(level, nil)
		if err != nil {
			t.Fatalf("Policy(%q) error = %v, want nil", level, err)
		}
		if policy != nil {
			t.Errorf("Policy(%q) policy = %+v, want nil (field omitted)", level, policy)
		}
	}
}
